...
const char NAME_REPLAY_COUNT[] = "ReplayCount"; // corresponding to the name in "Sample.cfg"
...
bool TraceConfig::setConfig(const char *name, const char *value) {
  ...
  else if (MATCH_NAME(NAME_REPLAY_COUNT)) {
      replayCount = strtoul(value, nullptr, 10);
  }
  else {
    ret = false;
  }
