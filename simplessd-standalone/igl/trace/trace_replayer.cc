...
namespace IGL {
//TraceReplayer* TraceReplayer::_instance = NULL;
//TraceReplayer* TraceReplayer::Instance(){
//    return _instance;
//}
...
//  _instance = this;
  ...
  filename = c.readString(CONFIG_TRACE, TRACE_FILE);
  SimpleSSD::FTL::PageMapping::Instance()->filename = filename;
  file.open(filename);
...
//  replay_count = c.readUint(CONFIG_TRACE, TRACE_REPLAY_COUNT);
  replay_count = 1500;
  max_io = c.readUint(CONFIG_TRACE, TRACE_IO_LIMIT); //0 when I/O count is unlimited
...
void TraceReplayer::getProgress(float &val) {
  if (max_io == 0) {
    ...
    val = ((float)ptr + replayed_count * fileSize) / (fileSize * read_count);
  }
  else {
...
void TraceReplayer::parseLine() {
  ...
  while (true) {
...

    {
      ...
      if (file.eof()) { // trace replay module
          replayed_count++;
          SimpleSSD::FTL::PageMapping::Instance()->replayed_count++;

          if (replayed_count >= replay_count) {
              eof = true;
          } else {
//               file.seekg(0, std::ios::beg);
              file.close();
          ...
          std::cout<< filename << " replayed_count: " << replayed_count << std::endl;
      }
      std::getline(file, line);
...
//      std::cout<< "getmatchedline: " << line << std::endl;
      break;
    }
  }
  ...
  // Timestamp for trace repetition in the strict mode
  if (replayed_count == 0) {
      tick_offset = linedata.tick;
  } else {
      linedata.tick += tick_offset * replayed_count;
  }
...
void TraceReplayer::submitIO() { // submit io and prepare the next one
  BIL::BIO bio;
  ...
  io_depth++;
  io_submitted = io_submitted + linedata.length; //record submitted io size
