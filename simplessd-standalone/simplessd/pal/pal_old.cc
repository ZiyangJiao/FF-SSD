...
void PALOLD::getStatValues(std::vector<double> &values) {
  ...
  values.push_back(stat.readCount * param.pageSize); // pal.read.bytes = readCount * pageSize (including all reads - host,GC,WL,etc)
  values.push_back(stat.writeCount * param.pageSize); // pal.write.bytes = writeCount * pageSize (including all write - host,GC,WL,etc)
  values.push_back(stat.eraseCount * param.pageSize * param.page); // pal.erase.bytes = eraseCount * pageSize (including all erase -- host,GC,WL,etc)
...
uint64_t PALOLD::getEraseCount(){
    return stat.eraseCount;
}

uint64_t PALOLD::getWriteCount(){
    return stat.writeCount;
}
