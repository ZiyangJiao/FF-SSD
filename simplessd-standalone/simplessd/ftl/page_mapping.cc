...
int* PageMapping::lastRecord = nullptr;
int PageMapping::count = 0;
int PageMapping::observation = 0;
uint64_t PageMapping::hostWriteCount = 0;

uint32_t PageMapping::replayed_count = 0;
PageMapping* PageMapping::_instance = nullptr;
std::string PageMapping::filename = "";
PageMapping* PageMapping::Instance() {
    return _instance;
}
...
PageMapping::PageMapping(ConfigReader &c, Parameter &p, PAL::PAL *l,
  ...
  lastRecord = new int[param.totalPhysicalBlocks];
  for (uint32_t i = 0; i < param.totalPhysicalBlocks; i++) { // block index starts from 0
...
PageMapping::~PageMapping() {
    std::ofstream myfile;
    myfile.open(filename+".ec", std::ios_base::app);
    int tmpRecord[param.totalPhysicalBlocks] = { };
    for (auto &iter : blocks) {
        tmpRecord[iter.second.getBlockIndex()] = iter.second.getEraseCount();
    }
    for (auto riter = freeBlocks.rbegin(); riter != freeBlocks.rend(); riter++) {
        int eraseCnt = riter->getEraseCount();
        tmpRecord[riter->getBlockIndex()] = eraseCnt;
    }
    for (auto eraseCnt : tmpRecord) {
        myfile << eraseCnt << " ";
    }
//    for (auto &iter : blocks) {
//        int eraseCnt = iter.second.getEraseCount();
//        myfile << eraseCnt << " ";
//    }
//    for (auto riter = freeBlocks.rbegin(); riter != freeBlocks.rend(); riter++) {
//        int eraseCnt = riter->getEraseCount();
...
bool freelistsort(const Block& a, const Block& b) {
...
void PageMapping::write(Request &req, uint64_t &tick) {
  ...
  if (int(replayed_count) > count) { // whole 256GB = 67108864 pages(4K)
      count++;
      observation++;

      if (true) { // log
          int tmpRecord[param.totalPhysicalBlocks] = { };
//          std::cout << "param.totalPhysicalBlocks: " << param.totalPhysicalBlocks << std::endl;
          std::ofstream myfile;
          myfile.open(filename+".ec", std::ios_base::app);
//          myfile.open(filename+".ec2", std::ios_base::app);
          for (auto &iter : blocks) {
              tmpRecord[iter.second.getBlockIndex()] = iter.second.getEraseCount();
          }
          for (auto riter = freeBlocks.rbegin(); riter != freeBlocks.rend(); riter++) {
      ...
      if (true && observation >= 2) { // prediction
          int tmpRecord[param.totalPhysicalBlocks] = {};
          int ret = -1;
          if (filename.find("BuildServer") != std::string::npos) {
              ret = system("bash /home/zjiao04/simplessd-standalone/FF/runpy_FF_BuildServer.sh");
          } else if (filename.find("DevelopmentToolsRelease") != std::string::npos) {
              ret = system("bash /home/zjiao04/simplessd-standalone/FF/runpy_FF_DevelopmentToolsRelease.sh");
          } else if (filename.find("DisplayAdsDataServer") != std::string::npos) {
              ret = system("bash /home/zjiao04/simplessd-standalone/FF/runpy_FF_DisplayAdsDataServer.sh");
          } else if (filename.find("DisplayAdsPayload") != std::string::npos) {
            ret = system("bash /home/zjiao04/simplessd-standalone/FF/runpy_FF_DisplayAdsPayload.sh");
          } else if (filename.find("LiveMapsBackEnd") != std::string::npos) {
              ret = system("bash /home/zjiao04/simplessd-standalone/FF/runpy_FF_LiveMapsBackEnd.sh");
          } else if (filename.find("MSNStorageCFS") != std::string::npos) {
              ret = system("bash /home/zjiao04/simplessd-standalone/FF/runpy_FF_MSNStorageCFS.sh");
          } else if (filename.find("MSNStorageFileServer") != std::string::npos) {
              ret = system("bash /home/zjiao04/simplessd-standalone/FF/runpy_FF_MSNStorageFileServer.sh");
          } else if (filename.find("RadiusAuthentication") != std::string::npos) {
              ret = system("bash /home/zjiao04/simplessd-standalone/FF/runpy_FF_RadiusAuthentication.sh");
          } else if (filename.find("RadiusBackEndSQLServer") != std::string::npos) {
              ret = system("bash /home/zjiao04/simplessd-standalone/FF/runpy_FF_RadiusBackEndSQLServer.sh");
          } else {
              std::cout << "Prediction Module Not Found!" << std::endl;
          }

          if (ret != -1) {
              std::ifstream file(filename + ".ec");
//              std::ifstream file(filename + ".ec2");
              std::string line;
              std::string tmp;
              if (file.is_open()) {
                  while (std::getline(file, tmp) && tmp != "") {
                      line = tmp;
                  }
                  file.close();
              } else {
                  std::cout << "EC Log File Not Found!" << std::endl;
              }
              ...
              int cnt = 0;
              while (pch != NULL) {
                  tmpRecord[cnt] = atoi(pch);
                  cnt++;
                  pch = strtok(NULL, " ");
              }
              std::cout << "The number of blocks from ec log: " << cnt << std::endl;

              for (auto &iter : blocks) {
                  int index = iter.second.getBlockIndex();
                  iter.second.eraseCount = tmpRecord[index];
              }
              for (auto riter = freeBlocks.rbegin(); riter != freeBlocks.rend(); riter++) {
                  int index = riter->getBlockIndex();
                  riter->eraseCount = tmpRecord[index];
              }
              freeBlocks.sort(freelistsort);
          }
      }
  }
...
void PageMapping::writeInternal(Request &req, uint64_t &tick, bool sendToPAL) {
  ...
  for (uint32_t idx = 0; idx < bitsetSize; idx++) {
//    std::cout<<"bitsetSize: "<<bitsetSize<<std::endl;
    if (req.ioFlag.test(idx) || !bRandomTweak) {
...
void PageMapping::eraseInternal(PAL::Request &req, uint64_t &tick) {
  ...
  // Erase block
  block->second.erase(); // block->eraseCount++
